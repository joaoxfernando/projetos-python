...
def calcula_digito(cnpj, digito):
    ...
    for index, regressivo in enumerate(regressivos):
        soma += int(cnpj[index]) * regressivo
    digito = 11 - (soma % 11)
    digito = digito if digito <= 9 else 0

    return f'{novo_cnpj}{digito}'

def checa_sequencia(cnpj):
    sequencia = cnpj[0] * len(cnpj) == cnpj
    if sequencia == cnpj:
        return True
    else:
        return False
...
def gera():
    ...
    quarto_bloco = '0001'
    cnpj_sem_digitos = f'{primeiro_digito}{segundo_digito}{segundo_bloco}{terceiro_bloco}{quarto_bloco}00'
    novo_cnpj = calcula_digito(cnpj=cnpj_sem_digitos, digito=1)
    novo_cnpj = calcula_digito(cnpj=novo_cnpj, digito=2)
...
def formata(cnpj):
    ...
    return f'O CNPJ gerado é: {formatado}'
